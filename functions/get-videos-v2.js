...
const PAGE_SIZE = parseInt(Deno.env.get('PAGE_SIZE') ?? '5');
// Helper functions
...
const buildVideoHtml = async (video, availableLangs, languageMap)=>{
  ...
  for (const lang of availableLangs){
    ...
    tracksHtml.push(`<track kind="subtitles" src="${trackUrl}" srclang="${lang}" label="${nativeName}" ${defaultAttr}>`);
  }
...
const processVideoUrls = async (video, termTranslationMap, languageMap)=>{
  ...
  if (video.title_translations) {
    try {
      const translations = typeof video.title_translations === 'string' ? JSON.parse(video.title_translations) : video.title_translations;
...
Deno.serve(async (req)=>{
  try {
    ...
    const { data: videos, error: videoError } = await supabase.from('videos').select("id, title, keyword, file_size, duration, width, height, aspect_ratio, title_translations, tags, categories, store_dir, filename").gt('id', lastId).eq('status', 'published').order('id', {
      ascending: true
    }).limit(PAGE_SIZE);
    ...
    if (allTerms.size > 0) {
      const { data: translations, error: translationError } = await supabase.from('terms').select('id, text, lang, translation').in('text', Array.from(allTerms));
      if (translationError) throw translationError;
      translationMap = createTermTranslationMap(translations);
    }
    // Get all languages and create a map
    const { data: languages, error: languageError } = await supabase.from('languages').select('code, native_name');
    ...
    const processedVideos = await Promise.all(videos.map((video)=>processVideoUrls(video, translationMap, languageMap)));
